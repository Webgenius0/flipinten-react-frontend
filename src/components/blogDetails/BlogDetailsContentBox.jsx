        ...
        <div>
          <div className="w-[70%] bg-white p-10 rounded-3xl">
            ...
            <div className="w-full h-[473px] rounded-2xl overflow-hidden mt-6">
                <img className="w-full h-full object-cover" src={BlogDetailsBannerTwo} alt="not found" />
            </div>
            {/* Country Tag */}
            <div className="mt-6 flex gap-2 items-center">
                {CountryData.map((country)=>(
                    <div key={country.id} className="py-1 px-3 bg-[#E7E7E7] rounded-[16px] w-max text-[12px] leading-[18px] capitalize">{country.name}</div>
                ))}
            </div>
            ...
            <div className="mt-[28px] flex flex-col gap-6 pb-6 border-b border-dashed">
                ...
                <p className="text-[#4F4F4F] text-[16px] leading-6">Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit.</p>
            </div>

            {/* Table of content */}
            <div className="pb-6 border-b border-dashed">
                <div className="mt-6 bg-[#E7E7E7] p-8 rounded-2xl w-max">
                    <h2 className="text-[#0B0B0B] text-2xl font-semibold leading-8 mb-6">Table of content</h2>
                    <ul className="flex flex-col gap-3 pl-4">
                        {TableContentData.map((item)=>(
                            <li key={item.id} className=" list-disc ">
                                <Link className="text-[#0B0B0B] text-[18px] font-medium leading-[27px] underline">{item.text}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
